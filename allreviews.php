...
";
            ...
            if (isset($_POST["but"])) {
                ...
                $ImgSql = "SELECT `id`, `name`, `image` FROM `products` WHERE `id`=$phoneId  ";


                $sql1 = "SELECT count(`id`) as cnt FROM `reviews` WHERE `prod_id`=$phoneId  and `polarity`=0 and active=1 ";
                $sql2 = "SELECT count(`id`) as cnt FROM `reviews` WHERE `prod_id`=$phoneId  and `polarity`>0 and active=1";
                $sql3 = "SELECT count(`id`) as cnt FROM `reviews` WHERE `prod_id`=$phoneId  and `polarity`< 0 and active=1";
                $result1 = $connection->query($sql1);
                $result2 = $connection->query($sql2);
                $result3 = $connection->query($sql3);


                $resultImg = $connection->query($ImgSql);
                ...
                <tr>
                <td> Image </td>
                <td> <img src='./$getimage' alt='' class='img-thumbnail' height=400 width=200>
                </td>
  ...
  <img src='neutral.png' width='20' height='20'>
  <h6 class='card-title'> Neutral Reviews </h6>
    <h4 class='card-title'><p class='text-dark'>$cnt1  </p>  </h4> ";


                echo " 
                <img src='postive.png' width='20' height='20'>

                
                <h6 class='card-title'>Positive Reviews </h6>

    <h4 class='card-title'>  <p class='text-success'>$cnt2   </p></h4>

    <img src='negative.png' width='20' height='20'>

    <h6 class='card-title'> Negative Reviews </h6>
    <h4 class='card-title'>
    <p class='text-danger'>$cnt3 </p>
    </h4>
